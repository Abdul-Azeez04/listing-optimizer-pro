...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { sequence_type, product_name, product_description } = body;

    if (!sequence_type || !product_name || !product_description) {
    ...
    const typeLabel = sequence_type.replace(/_/g, ' ');
    ...
    const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are CONVRT.AI, an expert email marketing copywriter for ecommerce sellers. Write high-converting email sequences. Respond with valid JSON only." },
          { role: "user", content: `Write a 4-email ${typeLabel} sequence for this product.\n\nPRODUCT: ${product_name}\nDESCRIPTION: ${product_description}\n\nReturn JSON: { "emails": [{ "subject": string, "body": string }, ...] }` },
        ],
        temperature: 0.7,
        max_tokens: 3000,
      }),
    ...
    await supabaseAdmin.from("email_sequences").insert({
      user_id: userId, sequence_type, product_name, emails: result.emails,
    });
