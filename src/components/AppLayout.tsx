...
import { LayoutDashboard, Sparkles, Clock, Settings, LogOut, Menu, X } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'New Rewrite', path: '/rewrite', icon: Sparkles },
    ...
    <div className="flex min-h-screen bg-background">
      ...
      {sidebarOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-64 bg-card border-l border-border p-6 space-y-4">
            {navItems.map((item) => {
              const active = location.pathname === item.path;
              return (
                <Link
            ...
            <button
              onClick={signOut}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mt-8"
            >
